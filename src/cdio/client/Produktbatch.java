...
//					Tilføj ny receptkomponent
					...
					public void onClick(ClickEvent event) {
						...
						client.service.createPBKomp(new ProduktBatchKompDTO(Integer.parseInt(KomponentHandler.this.produktBatchlist.getSelectedItemText()), Integer.parseInt(KomponentHandler.this.raavareBatchlist.getSelectedItemText()), 0, 0, 0), new AsyncCallback<Void>() {
									...
									boolean addable = true;
									
									 {
										for (int j = 0; j < raavareBatchesIRec.size(); j++) {
											if (result.get(i).getRaavareNavn().equalsIgnoreCase(raavareBatchesIRec.get(j))){
												addable = false;
										}
