...
public class MYSQLOperatoerDAO implements OperatoerDAO {
	...
	public UserDTO getOperatoer(int oprId) throws DALException {
		try {
			CallableStatement getOP = Connector.getInstance().getConnection()
	...
	public void createOperatoer(UserDTO opr) throws DALException {
		try {
			CallableStatement createOP = Connector.getInstance().getConnection()
	...
	public void updateOperatoer(UserDTO opr) throws DALException {
		try {
			CallableStatement updateOP = Connector.getInstance().getConnection()
	...
	public List<UserDTO> getUserList(String token) throws DALException {
		...
		try {
			...
			if (token.equals("admin")) {
				...
				while (rs.next()) {
					UserDTO current = new UserDTO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
