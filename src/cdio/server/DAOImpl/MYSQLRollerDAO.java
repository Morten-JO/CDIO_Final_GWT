...
public class MYSQLRollerDAO implements RollerDAO {
	...
	public void createRolle(RollerDTO rolle) throws DALException {
		try {
			CallableStatement createRolle = Connector.getInstance().getConnection()
	...
	public void updateRolle(RollerDTO opr, String rolle) {
		try {
			CallableStatement updateRolle = Connector.getInstance().getConnection()
					.prepareCall("call update_roller(?,?)");
